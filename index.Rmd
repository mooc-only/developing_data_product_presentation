...
```{r using_interactive_plots, echo = FALSE, results='hide'}
data(mtcars)
mtcars$am = as.factor(ifelse(mtcars$am == 1, "Manual", "Automatic"))
# applied to an example of ramnathv himself
plottingData = setNames(
  as.data.frame(boxplot(mpg ~ am, data = mtcars, plot = F)$stats),
  nm = NULL
  )

library(rCharts)
am.Plot <- Highcharts$new()
am.Plot$set(series = list(list(
  name = "Observations",
  data = plottingData))
  )
am.Plot$xAxis(
  categories = levels(mtcars$am),
  title = list(text = "Transmission")
  )
am.Plot$yAxis(
  title = list(text = "Miles per Gallon")
  )
am.Plot$chart(type = 'boxplot')

# This is to use with direct html code insertion
am.Plot$save("assets/img/transmission_boxplot.html", cdn = TRUE)
```

```{r insert_plot, echo=FALSE, results='asis'}
# Should be a better and easy way to do all this
cat('<iframe src="assets/img/transmission_boxplot.html" width=50% height=50%></iframe>')

#am.Plot$print('iframesrc', include_assets=TRUE, cdn = TRUE)
#am.Plot$show("iframesrc", cdn = TRUE)
```
